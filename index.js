...
function addValidationOfObjectSchema(ctx, schemaKey, { allowUnknown = false, properties, required = [], }) {
    return ctx.addValidation(function* (locals, name, value, depth, errors) {
        ...
        const shouldCheckProperties = !allowUnknown ||
...
const DefaultMessage = {
    ...
    arrayMinLength: ({ name, minLength }) => minLength === 1
    ...
    bigintMaxValue: ({ name, maxValue }) => `"${name}" must be less than or equal to ${maxValue}n.`,
    bigintMinValue: ({ name, minValue }) => `"${name}" must be greater than or equal to ${minValue}n.`,
    boolean: ({ name }) => `"${name}" must be a boolean value.`,
    class: ({ name, constructor: ctor }) => `"${name}" must be an instance of ${ctor.name || "Anonymous class"}.`,
    custom: ({ name, checkName }) => `"${name}" must be ${checkName}.`,
    enum({ name, values }) {
        const options = values.map(valueToString$1).filter(isNotDuplicateValue);
        if (options.length <= 2) {
    ...
    numberMaxValue: ({ name, maxValue }) => `"${name}" must be less than or equal to ${maxValue}.`,
    numberMinValue: ({ name, minValue }) => `"${name}" must be greater than or equal to ${minValue}.`,
    ...
    stringMinLength: ({ name, minLength }) => minLength === 1
        ? `"${name}" must not be empty.`
    ...
    tupleLength: ({ name, length }) => `"${name}" must contain exactly ${plural(length, "item")}.`,
    union({ name, schemas }) {
        const options = []
 ...
 */
function createValidation(schema, { defaultMessages = DefaultMessage } = {}) {
    const validateFn = createValidationOfSchema(schema);
    return function validate(value, { messages = defaultMessages, name = "value" } = {}) {
 ...
 * validation.
 */
function validate(schema, value, { messages = DefaultMessage, name = "value" } = {}) {
